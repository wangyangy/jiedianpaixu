...
//	static HashMap<Integer,ArrayList<Integer>> map = new HashMap<Integer, ArrayList<Integer>>();
	static ConcurrentHashMap<Integer,ArrayList<Integer>> map = new ConcurrentHashMap<Integer, ArrayList<Integer>>();
		...
		for(int v=0;v<=nodes;v++)
		{
			if(map.get(u).contains(v))
			{
				...
				if(vis[v]==0)
				{
					//标记节点已经搜索过
					vis[v]=1;
//					System.out.println("线程："+Thread.currentThread()+"深度："+deep);
					++deep;
